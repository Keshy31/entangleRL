...
args['train']['batch_size'] = 4096  # Double your 8192; larger batches = longer GPU kernels, better util. Auto-adjusts with bptt.
args['train']['bptt_horizon'] = 'auto'  # Keep—long enough for quantum sequences (e.g., 5-10 gates to Bell state) without OOM.
args['train']['minibatch_size'] = 512  # Double your 256; processes more data per gradient step for stability.
args['train']['max_minibatch_size'] = 1024  # Double your 1024; cap to avoid VRAM spikes on RTX 4080.
...
args['vec']['num_envs'] = 64  # 4x your 32; more parallel sims = faster data collection, better GPU fill. QuantumPrepEnv is lightweight (2 qubits), so this fits.
...
writer = SummaryWriter(log_dir='logs/tensorboard/quantum_prep_rnn_500k')  # Create a log dir; run tensorboard --logdir=logs/tensorboard to view
...
## Save the policy
torch.save(trainer.policy.state_dict(), 'models/ppo_quantum_rnn_500k_no_noise.pth')
