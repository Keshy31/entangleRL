...
args['train']['batch_size'] = 3072  # Double your 8192; larger batches = longer GPU kernels, better util. Auto-adjusts with bptt.
args['train']['bptt_horizon'] = 'auto'  # Keep—long enough for quantum sequences (e.g., 5-10 gates to Bell state) without OOM.
args['train']['minibatch_size'] = 1536  # Double your 256; processes more data per gradient step for stability.
args['train']['max_minibatch_size'] = 3072  # Double your 1024; cap to avoid VRAM spikes on RTX 4080.
...
args['train']['vf_coef'] = 0.4  # Balances value loss; up if explained_variance stays low.
...
args['vec']['num_envs'] = 96  # 4x your 32; more parallel sims = faster data collection, better GPU fill. QuantumPrepEnv is lightweight (2 qubits), so this fits.
args['vec']['num_workers'] = 12  # Double your 8; spreads CPU load (QuTiP sims are CPU-bound by default).
args['vec']['batch_size'] = 8  # 8x your 4; aligns with num_envs for efficient vec rollouts (masks dones automatically).
...
while trainer.global_step < total_timesteps:  # Use variable for brevity
    ...
    trainer.config['ent_coef'] = max(0.0, 0.01 * (1 - progress_ratio))  # Clamp to >=0
